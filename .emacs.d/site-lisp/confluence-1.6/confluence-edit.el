...
(defmacro my-def-headline-face (level height foregrnd-on-dark foregrnd-on-light weight)
  `(defface ,(intern (concat "my-headline-face-" level))
     '((((class color) (background dark))
        (:foreground ,foregrnd-on-dark :weight ,weight :height ,height))
       (((class color) (background light))
        (:foreground ,foregrnd-on-light :weight ,weight :height ,height))
       (t (:weight ,weight :height ,height)))
     ,(concat "Font Lock Mode face used for " level " headline in confluence pages.")
     :group 'confluence-faces))

(let ((letcol-alist '((h1 2.5) (h2 2.0) (h3 1.5) (h4 1.0))))
  (loop for elem in letcol-alist
    for level = (format "%s" (car elem))
    for height = (cadr elem)
    do 
    (eval (macroexpand `(my-def-headline-face 
                          ,level ,height 
                          "yellow" "midnight blue" extra-bold)))))
...
(defconst confluence-font-lock-keywords-1
  (list
   ...
   ;; headings
   '("^h1[.] \\(.*?\\)\\s-*$"
     (1 'my-headline-face-h1 t))
   '("^h2[.] \\(.*?\\)\\s-*$"
     (1 'my-headline-face-h2 t))
   '("^h3[.] \\(.*?\\)\\s-*$"
     (1 'my-headline-face-h3 t))
   '("^h[4-9][.] \\(.*?\\)\\s-*$"
     (1 'my-headline-face-h4 t))
;;   '("^h2[.] \\(.*?\\)\\s-*$"
